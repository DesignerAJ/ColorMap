...
**영국**도 같은 방법으로 뽑습니다(`admin1-lines/GBR.json`,
`node recorder/tools/build-admin1-lines.mjs`). 여기는 정밀도가 아니라 **단위**가 문제였습니다 —
Mapbox의 1급 행정구역은 영국을 넷(잉글랜드·스코틀랜드·웨일스·북아일랜드)으로만 나누는데
우리 데이터에는 카운티·단일자치체·런던 자치구까지 232개가 들어 있어서, 색칠은 232개로
되는데 선은 4개만 그어졌습니다. 다른 나라를 늘리려면 그 ISO3로 도구를 한 번 돌리고
`recorder.js`의 `ADMIN1_LINE_COUNTRIES`에 한 줄 넣으면 됩니다. 이웃 폴리곤이 꼭짓점을
공유하는 출처여야 하며(영국은 64%), 안 되면 도구가 막습니다.
출처는 그 나라 폴리곤과 같습니다 — 영국은 Natural Earth(퍼블릭 도메인)라 표기 의무가
새로 생기지 않습니다.
