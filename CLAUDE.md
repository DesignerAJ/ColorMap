...
삼각형의 원인이 **일곱**이고 증상이 똑같다 — 분기점 조립, 핀치, 자기교차, 맞닿은 구멍,
구멍 안의 구멍, 해안선 잇기 실패, 감김 방향. 하나를 고쳐도 안 없어지면 나머지를 의심할 것.
...
node --test test/*.test.js            # 74개, 설치 불필요
npm install && node --test test/dom/*.test.js   # 43개, jsdom 필요
```
